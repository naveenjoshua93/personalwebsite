...
**Rename decided and executed: Teclato.** The owner asked for a cute name starting with "tec" and ending in "to" or "y" after tecato.com turned out to be owned by a third party since 2005. Candidates checked against the Verisign registry (tecito, tecly, tecky, tecsy, tecjoy, tecnito all taken); teclato.com was unregistered at decision time and needs registering by the owner. Rebranded across wordmark (Tec in cyan), all copy and titles, JSON-LD, film title cards (TECLATO ⚡ FILM NN), the mascot's chest badge (TW to TL, mascot still named Brick), the OG card, email (hello@teclato.com), and social placeholder handles (/company/teclato, teclato.substack.com, @teclato). "War room" survives as product language: the rooms, the films, the armory. Canonical host stays the workers.dev URL until teclato.com is attached.

**2026-08-31 additions:** the pain section now has two labelled strips matching the doors: "If you're raising" (runway clock, polite passes, the partner meeting without you) and "If you're selling" (the original enterprise panels), converging on one wide Brick twist banner ("Whichever door you came through"). The slogan "We only take must-win deals" is locked under the wordmark in the header on every page.

## Specimen 001: the annotated pitch deck (2026-09-01)

**The Armory's first real artifact.** A complete 12-slide Series A deck for Mortar, an invented AI-agents-for-finance-ops startup, chosen with the owner (AI agents sector, Series A stage, X-ray format, "the argument" lens). Live at /armory/specimen-pitch-deck/ with a real downloadable PPTX at /files/mortar-specimen-deck.pptx (generated by scripts/specimen-deck.cjs via pptxgenjs).

- The deck deliberately uses its own quiet, investor-grade design language (paper white, ink, cobalt, thin rules), distinct from the Teclato chrome around it. Range is the point: the loud site frames a calm deck.
- The X-ray toggle overlays each slide with the argument it makes and its chain links to the previous and next slide. Keyboard: arrows navigate, X toggles.
- Honesty mechanics: every slide carries a FICTIONAL SPECIMEN stamp, fictional numbers are internally consistent (14 customers x ~$120k ACV = $1.7M ARR) and footnoted as fictional, and the team slide uses unnamed fictional silhouettes because inventing credentialed humans is where specimen decks go wrong.
- The twelve-slide argument chain is itself the skill display: one-liner, problem, why-now, mechanism, wedge, three corroborating numbers, compounding assets instead of a moat claim, an honest field table instead of a fake quadrant, pricing-as-strategy, receipts-not-logos team, ask-as-plan, close-on-the-next-round.
- Home Armory card updated to link to it; first working "Grab it" on the site.
